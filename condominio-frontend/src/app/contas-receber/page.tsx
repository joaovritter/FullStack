...
export default function ContasReceberPage() {
  ...
  return (
    <PageLayout
      ...
      description="Gestão de receitas e recebimentos gerais."
    >
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              placeholder="Ex: 5" value={form.ID_MORADOR}
              onChange={e => setForm({ ...form, ID_MORADOR: e.target.value })}
            />
          </div>
          <div className="flex-[2]">
            <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">
              Descrição da Receita <span className="text-[#A0006D]">*</span>
            </label>
            <input
              required
              className="w-full border border-slate-200 bg-white px-4 py-3 rounded-xl focus:outline-none focus:border-[#4A8BDF] focus:ring-4 focus:ring-[#4A8BDF]/10 transition-all font-medium text-slate-800 placeholder-slate-400"
              placeholder="Ex: Multa, Reserva..." value={form.DESCRICAO}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full relative">
            ...
            <input
              ...
              className="w-full border border-slate-200 bg-white px-4 py-3 rounded-xl focus:outline-none focus:border-[#4A8BDF] focus:ring-4 focus:ring-[#4A8BDF]/10 transition-all font-medium text-slate-800 placeholder-slate-400"
              placeholder="Ex: 150.00" value={form.VALOR}
          ...
          <div className="md:ml-auto w-full md:w-auto mt-4 md:mt-0">
            ...
            >
              {editId ? "Atualizar" : "Salvar Receita"}
            </button>
          </div>
        </div>
      </form>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b-2 border-slate-100">
              <th className="py-4 px-6 text-xs font-bold text-[#A0006D] uppercase tracking-widest bg-slate-50 rounded-tl-xl">ID Recebimento</th>
              <th className="py-4 px-6 text-xs font-bold text-[#A0006D] uppercase tracking-widest bg-slate-50">Descrição / Morador</th>
          ...
          <tbody>
            {contas.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-slate-500 font-medium">Nenhuma conta a receber registrada.</td>
              </tr>
            ) : (
              contas.map((c: any) => (
                <tr key={c.ID_CONTA_RECEBER} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                  <td className="py-4 px-6 font-bold text-slate-800">#{c.ID_CONTA_RECEBER}</td>
                  <td className="py-4 px-6 font-medium text-slate-800">
                    <span className="block">{c.DESCRICAO}</span>
                    <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-xs font-bold w-fit mt-1 block">
                      Morador {c.ID_MORADOR}
                    </span>
                  </td>
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => handleDelete(c.ID_CONTA_RECEBER)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Excluir">
